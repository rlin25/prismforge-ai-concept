-- Phase 2 Multi-Agent Validation System (SAFE TABLE AND COLUMN CREATION)
-- PrismForge AI - Professional M&A Validation Platform

-- Create tables first with all required columns
...
CREATE TABLE IF NOT EXISTS analysis_sessions (
    ...
    execution_id TEXT DEFAULT gen_random_uuid()::TEXT, -- For real-time tracking
...
-- Now add any missing columns to existing tables (safe operations)
-- Add execution_id to analysis_sessions if it doesn't exist
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS execution_id TEXT DEFAULT gen_random_uuid()::TEXT;
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS started_at TIMESTAMPTZ;
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ;
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS total_processing_time_ms INTEGER;
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS overall_recommendation TEXT;
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS confidence_score DECIMAL(3,2);
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS professional_quality_score DECIMAL(3,2);
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS professional_standard_met BOOLEAN DEFAULT FALSE;
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS tokens_remaining INTEGER;
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS cost_breakdown JSONB DEFAULT '{}';

-- Add missing columns to agent_executions if they don't exist
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS started_at TIMESTAMPTZ;
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ;
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS processing_time_ms INTEGER;
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS findings JSONB DEFAULT '[]';
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS risk_summary TEXT;
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS confidence_score DECIMAL(3,2);
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS professional_quality_score DECIMAL(3,2);
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS professional_standard_met BOOLEAN DEFAULT FALSE;
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS tokens_remaining INTEGER;
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS api_calls JSONB DEFAULT '[]';
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS api_call_count INTEGER DEFAULT 0;
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS context_for_next_agent JSONB DEFAULT '[]';
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS error_message TEXT;
ALTER TABLE agent_executions ADD COLUMN IF NOT EXISTS retry_count INTEGER DEFAULT 0;

-- Update execution_id to be NOT NULL and UNIQUE
UPDATE analysis_sessions SET execution_id = gen_random_uuid()::TEXT WHERE execution_id IS NULL;
ALTER TABLE analysis_sessions ALTER COLUMN execution_id SET NOT NULL;

-- Create unique constraint if it doesn't exist
CREATE UNIQUE INDEX IF NOT EXISTS analysis_sessions_execution_id_unique ON analysis_sessions(execution_id);
...
DROP POLICY IF EXISTS "analysis_sessions_organization_access" ON analysis_sessions;
DROP POLICY IF EXISTS "agent_executions_organization_access" ON agent_executions;
DROP POLICY IF EXISTS "quality_validations_organization_access" ON quality_validations;
DROP POLICY IF EXISTS "agent_status_updates_public_read" ON agent_status_updates;
DROP POLICY IF EXISTS "professional_deliverables_organization_access" ON professional_deliverables;
...
-- FIXED: Function to initialize agent executions for new analysis sessions
...
BEGIN
    ...
    INSERT INTO agent_executions (
        ...
        NEW.id,
        NEW.execution_id, -- This now correctly references the execution_id column
        'skeptic',
...
-- Drop and recreate the trigger to initialize agent executions for new analysis sessions
DROP TRIGGER IF EXISTS initialize_agent_executions_trigger ON analysis_sessions;
...
BEGIN
    ...
    tokens_remaining = (
        SELECT token_budget - COALESCE(SUM(tokens_used), 0) FROM agent_executions 
        WHERE analysis_session_id = NEW.analysis_session_id
...
-- Drop and recreate trigger to update token calculations
DROP TRIGGER IF EXISTS update_tokens_remaining_trigger ON agent_executions;
